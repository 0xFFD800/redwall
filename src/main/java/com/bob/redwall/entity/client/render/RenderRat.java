...
import com.bob.redwall.entity.client.model.ModelRat;
...
public class RenderRat<T extends EntityAbstractNPC> extends RenderLiving<T> {
    public static final Factory FACTORY = new Factory();

    public RenderRat(RenderManager renderManager) {
        super(renderManager, new ModelRat(0.0F, 0.0F), 0.5F);
        this.addLayer(new LayerNPCHeldItem(this));
        this.addLayer(new LayerNPCArmor(this) {
        	@Override
            protected void initArmor() {
                this.modelLeggings = new ModelRat(0.5F, 0.0F);
                this.modelArmor = new ModelRat(1.0F, 0.0F);
            }
        });
    }
    ...
    public static class Factory implements IRenderFactory<EntityAbstractNPC> {
        ...
        public Render<? super EntityAbstractNPC> createRenderFor(RenderManager manager) {
            return new RenderRat<EntityAbstractNPC>(manager);
        } 
    }
}
