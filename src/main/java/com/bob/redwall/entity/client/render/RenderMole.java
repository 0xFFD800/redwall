...
import com.bob.redwall.entity.client.model.ModelMole;
...
public class RenderMole<T extends EntityAbstractNPC> extends RenderLiving<T> {
    public static final Factory FACTORY = new Factory();

    public RenderMole(RenderManager renderManager) {
        super(renderManager, new ModelMole(0.0F, 0.0F), 0.5F);
        this.addLayer(new LayerNPCHeldItem(this));
        this.addLayer(new LayerNPCArmor(this) {
        	@Override
            protected void initArmor() {
                this.modelLeggings = new ModelMole(0.5F, 0.0F);
                this.modelArmor = new ModelMole(1.0F, 0.0F);
            }
        });
    }
    ...
    public static class Factory implements IRenderFactory<EntityAbstractNPC> {
        ...
        public Render<? super EntityAbstractNPC> createRenderFor(RenderManager manager) {
            return new RenderMole<EntityAbstractNPC>(manager);
        } 
    }
}
