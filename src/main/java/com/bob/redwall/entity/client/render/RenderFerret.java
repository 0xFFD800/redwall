...
import com.bob.redwall.entity.client.model.ModelFerret;
...
public class RenderFerret<T extends EntityAbstractNPC> extends RenderLiving<T> {
	public static final Factory FACTORY = new Factory();

	public RenderFerret(RenderManager renderManager) {
		super(renderManager, new ModelFerret(0.0F, 0.0F), 0.5F);
		this.addLayer(new LayerNPCHeldItem(this));
		this.addLayer(new LayerNPCArmor(this) {
			@Override
			protected void initArmor() {
				this.modelLeggings = new ModelFerret(0.5F, 0.0F);
				this.modelArmor = new ModelFerret(1.0F, 0.0F);
			}
		});
	}
	...
	public static class Factory implements IRenderFactory<EntityAbstractNPC> {
		...
		public Render<? super EntityAbstractNPC> createRenderFor(RenderManager manager) {
			return new RenderFerret<EntityAbstractNPC>(manager);
		}
	}
}
