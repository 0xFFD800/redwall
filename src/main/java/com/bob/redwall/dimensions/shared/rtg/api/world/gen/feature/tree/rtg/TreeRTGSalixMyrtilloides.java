...
public class TreeRTGSalixMyrtilloides extends TreeRTG {
	...
	public TreeRTGSalixMyrtilloides() {
		...
		this.setLogBlock(BlockHandler.willow_log.getDefaultState()).setLeavesBlock(BlockHandler.willow_leaves.getDefaultState());
	}
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		for (int b = 0; b < branches; b++) {
			...
			while (c < branchLenght) {
				...
				this.placeLogBlock(world, new BlockPos(x + (int) (c * xd), y + (int) hd, z + (int) (c * yd)), this.trunkLog, this.generateFlag);
			}
			createLeavesAroundBranch(world, rand, x + (int) (c * xd), y + (int) hd, z + (int) (c * yd), 2, 1);
		}
