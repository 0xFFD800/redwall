 ...
 *
 *         Simplex-cellular noise is cellular noise implemented using the
 *         lattice of Simplex noise.
 *
 *         In this case the point contribution determination is implemented
 *         using a lookup scheme inspired by DigitalShadow's optimized
 *         implementation of OpenSimplex noise, and a permutation table of size
 *         1024 is used instead of the traditional 256.
 *
...
public class SimplexCellularOctave implements CellOctave {
	...
	public VoronoiResult eval(double x, double y) {
		...
		// Index to point list
		int index = ((int) (xsi + ysi) * 9) + ((int) (xsi * 2 - ysi + 1) * 9 * 2) + ((int) (ysi * 2 - xsi + 1) * 9 * 6);
		...
		for (int i = 0; i < 9; i++) {
			...
			double djx = jx - (c.dx + xi), djy = jy - (c.dy + yi);
		...
		if (crashing) {
			...
			if (otherSide == null)
				otherSide = complaint;
			else throw new RuntimeException(otherSide + "        " + complaint + " " + chunkManagerProblems);

			crashing = false;
		}
		if (results.shortestDistance > results.nextDistance)
			throw new RuntimeException();
