...
import com.bob.redwall.dimensions.shared.rtg.api.util.TimedHashMap;
 ...
 * noise,
 * 
 * Further modified to use saved points to avert basin centers being too close.
 * zeno410
 * 
 *
...
public class SpacedCellOctave implements CellOctave {
	 ...
	 * switch(distanceMethod) { case 0: return Math.sqrt(xDist * xDist + zDist *
	 * zDist) / SQRT_2; case 1: return xDist + zDist; default: return Double.NaN; }
	 * }
	 */
	...
	/*
	 * Working on a algorithm The algorithm is to create a set of points on a unit
	 * torus, none of which are too close. Each unit square gets a random subset of
	 * these points On hold since I found the problem was that valueNoise was using
	 * too small a divisor and letting points overlap
	 */
	...
	public static double valueNoise2D(int x, int z, long seed) {
		...
		// return 1.0 - ((double)((n * (n * n * 60493 + 19990303) + 1376312589) &
		// 0x7fffffff) / 1073741824.0);
	}

	TimedHashMap<Point, Point2D.Double[]> pointStorage = new TimedHashMap<Point, Double[]>(5 * 60 * 1000);
