...
import com.bob.redwall.dimensions.shared.rtg.api.util.PlateauBand;
...
public abstract class SurfaceBase {
	...
	protected IBlockState getShadowStoneBlock(IRTGWorld rtgWorld, int i, int j, int x, int y, int k) {
		...
		SurfaceEvent.HardcodedBlock event = new SurfaceEvent.HardcodedBlock(rtgWorld, i, j, x, y, k, shadowStoneBlock);
	...
	protected IBlockState getShadowDesertBlock(IRTGWorld rtgWorld, int i, int j, int x, int y, int k) {
		...
		SurfaceEvent.HardcodedBlock event = new SurfaceEvent.HardcodedBlock(rtgWorld, i, j, x, y, k, shadowDesertBlock);
		MinecraftForge.TERRAIN_GEN_BUS.post(event);
	...
	protected IBlockState hcCobble(IRTGWorld rtgWorld, int worldX, int worldZ, int chunkX, int chunkZ, int worldY) {
		...
		SurfaceEvent.HardcodedBlock event = new SurfaceEvent.HardcodedBlock(rtgWorld, worldX, worldZ, chunkX, chunkZ, worldY, cliffCobbleBlock);
		MinecraftForge.TERRAIN_GEN_BUS.post(event);
	...
	protected void initCliffBlocks() {
		cliffStoneBlock = getConfigBlock(biomeConfig.SURFACE_CLIFF_STONE_BLOCK.get(), biomeConfig.SURFACE_CLIFF_STONE_BLOCK_META.get(), Blocks.STONE.getDefaultState());

		cliffCobbleBlock = getConfigBlock(biomeConfig.SURFACE_CLIFF_COBBLE_BLOCK.get(), biomeConfig.SURFACE_CLIFF_COBBLE_BLOCK_META.get(), Blocks.COBBLESTONE.getDefaultState());
	}

	protected void initShadowBlocks() {
		shadowStoneBlock = getConfigBlock(rtgConfig.SHADOW_STONE_BLOCK_ID.get(), rtgConfig.SHADOW_STONE_BLOCK_META.get(), BlockUtil.getStateClay(9));

		shadowDesertBlock = getConfigBlock(rtgConfig.SHADOW_DESERT_BLOCK_ID.get(), rtgConfig.SHADOW_DESERT_BLOCK_META.get(), BlockUtil.getStateClay(0));
	}
}