...
public class VoronoiResult {
	...
	public final double interiorValue(double minimumDivisor) {
		// like interiorvalue but the value is capped to avert problems from too small
		// cells
		if (minimumDivisor > nextDistance)
			minimumDivisor = nextDistance;
	...
	public final Point2D.Float toLength(Point2D.Float toMap, float radius) {
		// returns a point on the vector from closestX, closestZ to the passed point but
		// at the specified
		// length
