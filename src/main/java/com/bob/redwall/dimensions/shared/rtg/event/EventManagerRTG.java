package com.bob.redwall.dimensions.shared.rtg.event;
...
import static net.minecraftforge.event.terraingen.OreGenEvent.GenerateMinable.EventType.SILVERFISH;

import java.util.ArrayList;
import java.util.WeakHashMap;

import com.bob.redwall.dimensions.redwall.RedwallBiomeProvider;
...
import com.bob.redwall.dimensions.shared.rtg.api.util.ModPresenceTester;
import com.bob.redwall.dimensions.shared.rtg.api.util.RandomUtil;
import com.bob.redwall.dimensions.shared.rtg.api.util.SaplingUtil;
...
import com.bob.redwall.dimensions.shared.rtg.world.gen.genlayer.RiverRemover;
...
public class EventManagerRTG {
    ...
    public class GenerateMinableRTG {
        ...
        public void generateMinableRTG(OreGenEvent.GenerateMinable event) {
            ...
            if (WorldTypeRedwall.chunkProvider == null) {
                ...
                RTG.eventMgr.setDimensionChunkLoadEvent(event.getWorld().provider.getDimension(), WorldTypeRedwall.chunkProvider.delayedDecorator);
                RTG.runOnNextServerCloseOnly(WorldTypeRedwall.chunkProvider.clearOnServerClose());

                Logger.debug("WorldTypeRTG#getChunkGenerator() returning ChunkProviderRTG");
            }
            ChunkOreGenTracker chunkOreGenTracker = WorldTypeRedwall.chunkProvider.getChunkOreGenTracker();
    ...
    public class InitBiomeGensRTG {
        ...
        public void initBiomeGensRTG(WorldTypeEvent.InitBiomeGens event) {
            ...
            // Are we in an RTG world?
            if (!(event.getWorldType() instanceof WorldTypeRedwall)) {
                return;
            }
    ...
    public class SaplingGrowTreeRTG {
        ...
        public void saplingGrowTreeRTG(SaplingGrowTreeEvent event) {
            ...
            if (!(DimensionManagerRTG.isValidDimension(event.getWorld().provider.getDimension())) ||
                !(event.getWorld().getBiomeProvider() instanceof RedwallBiomeProvider)) {
                return;
            }
