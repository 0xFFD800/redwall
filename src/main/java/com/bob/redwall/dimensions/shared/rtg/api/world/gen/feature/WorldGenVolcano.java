...
import com.bob.redwall.dimensions.shared.rtg.api.util.TerrainMath;
...
public class WorldGenVolcano {
	...
	private static final int lavaHeight = 138 + 3 + (RTGAPI.config().ENABLE_VOLCANO_ERUPTIONS.get() ? 5 : 0); // + 3 to
																												// account
																												// for
																												// lava
																												// cone
	...
	private static IBlockState volcanoBlock = getVolcanoBlock(RTGAPI.config().VOLCANO_BLOCK_ID.get(), RTGAPI.config().VOLCANO_BLOCK_META.get(), RTGConfig.DEFAULT_VOLCANO_BLOCK);
	private static IBlockState volcanoPatchBlock = getVolcanoBlock(RTGAPI.config().VOLCANO_MIX1_BLOCK_ID.get(), RTGAPI.config().VOLCANO_MIX1_BLOCK_META.get(), RTGConfig.DEFAULT_VOLCANO_MIX1_BLOCK);
	private static IBlockState volcanoPatchBlock2 = getVolcanoBlock(RTGAPI.config().VOLCANO_MIX2_BLOCK_ID.get(), RTGAPI.config().VOLCANO_MIX2_BLOCK_META.get(), RTGConfig.DEFAULT_VOLCANO_MIX2_BLOCK);
	@SuppressWarnings("deprecation")
	private static IBlockState volcanoPatchBlock3 = Blocks.STAINED_HARDENED_CLAY.getStateFromMeta(EnumDyeColor.BLACK.getMetadata());
	...
	public static void build(ChunkPrimer primer, World world, Random mapRand, int baseX, int baseY, int chunkX, int chunkY, OpenSimplexNoise simplex, CellNoise cell, float[] noise) {
		...
		for (int x = 0; x < 16; x++) {
			for (int z = 0; z < 16; z++) {
				...
				distanceEll = (float) TerrainMath.dis2Elliptic(i, j, baseX * 16, baseY * 16, simplex.noise2(i / 250f, j / 250f) * ventEccentricity, simplex.octave(1).noise2(i / 250f, j / 250f) * ventEccentricity);
									...
									if (distanceEll < 70 + simplex.noise2(x / 26f, y / 26f) * 5) {
										...
										if (mapRand.nextInt(1 + (int) Math.pow(Math.abs(distanceEll - (75 + simplex.noise2(x / 26f, y / 26f) * 5)), 1.5 + powerNoise) + 1) == 0) {
											if (mapRand.nextInt(20) == 0) {
												...
												b = Blocks.STONE.getDefaultState(); // Stone so that surfacing will run
																					// (so this usually becomes grass)
											}
										} else {
											b = volcanoBlock;
										}
									} else {
										b = Blocks.STONE.getDefaultState(); // Stone so that surfacing will run (so this
																			// usually becomes grass)
									}
								} else {
