...
import com.bob.redwall.dimensions.shared.rtg.api.world.biome.BiomeDecoratorRTG;
import com.bob.redwall.dimensions.shared.rtg.api.world.biome.IRealisticBiome;
import com.bob.redwall.dimensions.shared.rtg.api.world.biome.RealisticBiomeManager;
...
import com.bob.redwall.dimensions.shared.rtg.api.world.surface.SurfaceGeneric;
import com.bob.redwall.dimensions.shared.rtg.api.world.surface.SurfaceRiverOasis;
...
public abstract class RealisticBiomeBase implements IRealisticBiome {
	...
	public RealisticBiomeBase(Biome biome, Biome river) {
		...
		/*
		 * Disable base biome decorations by default. This also needs to be here so that
		 * ores get generated.
		 */
	...
	/*
	 * Returns the beach biome to use for this biome. By default, it uses the beach
	 * that has been set in the biome config. If automatic beach detection is
	 * enabled (-1), it uses the supplied preferred beach.
	 */
	...
	/*
	 * Returns the beach biome to use for this biome, with a dynamically-calculated
	 * preferred beach.
	 */
	...
	public float rNoise(RTGWorld rtgWorld, int x, int y, float border, float river) {
		...
		// set the lakeStrength to match river parameters. Changes are needed because
		// the
		// function is much steeper at the edges (rivers) than the center (lakes)
		float // lakeFlattening = lakeFlattening(lakeStrength, lakeWaterLevel,
				// lakeDepressionLevel);
		...
		/*
		 * if (riverFlattening <0) riverFlattening = 0; if (riverFlattening >1)
		 * riverFlattening = 1; if ((river<1)&&(lakeFlattening<1)) { riverFlattening =
		 * (1f-riverFlattening)/riverFlattening+(1f-lakeFlattening)/lakeFlattening;
		 * riverFlattening = (1f/(riverFlattening+1f)); } else if (lakeFlattening <
		 * riverFlattening) riverFlattening = lakeFlattening; // the lakes have to have
		 * a little less flattening to avoid the rocky edges lakeFlattening =
		 * lakeFlattening(lakeStrength, lakeWaterLevel, lakeShoreLevel);
		 */
	...
	public float erodedNoise(RTGWorld rtgWorld, int x, int y, float river, float border, float biomeHeight) {
		...
		if ((r < 1f && biomeHeight > 55f)) {
			float irregularity = rtgWorld.simplex.noise2(x / 12f, y / 12f) * 2f + rtgWorld.simplex.noise2(x / 8f, y / 8f);
	...
	public boolean compareTerrain(RTGWorld rtgWorld, TerrainBase oldTerrain) {
		...
		for (int x = -64; x <= 64; x++) {
			for (int z = -64; z <= 64; z++) {
				...
				// Logger.info("%s (%d) = oldNoise = %f | newNoise = %f",
				// this.baseBiome.getBiomeName(), Biome.getIdForBiome(this.baseBiome), oldNoise,
				// newNoise);

				if (oldNoise != newNoise) {
					throw new RuntimeException("Terrains do not match in biome ID " + Biome.getIdForBiome(this.baseBiome) + " (" + this.baseBiome.getBiomeName() + ").");
				}
			}
		}
