...
package com.bob.redwall.dimensions.shared.rtg.world.biome;

import com.bob.redwall.dimensions.shared.rtg.api.util.CircularSearchCreator;
...
public class BiomeAnalyzer {
    ...
    private boolean [] ravineBiome;
    private boolean [] beachBiome;
    private boolean [] landBiome;
    private int [] preferredBeach;
    private IRealisticBiome [] savedJittered;
    @SuppressWarnings("unused")
	private RealisticBiomeBase scenicLakeBiome = RealisticBiomeBase.getBiome(Biome.getIdForBiome(BiomeHandler.redwall_river));
    @SuppressWarnings("unused")
	private RealisticBiomeBase scenicFrozenLakeBiome = RealisticBiomeBase.getBiome(Biome.getIdForBiome(BiomeHandler.redwall_river_frozen));
    ...
    private void determineOceanBiomes() {
        ...
        for (int i = 0; i < oceanBiome.length; i++) {
            ...
            if (biome.getBiomeName().toLowerCase().contains("sea")) oceanBiome[i] = true;
            ...
            if (biome.getBiomeName().toLowerCase().contains("deeplough")) oceanBiome[i] = true;
        }
    ...
    private void determineRavineBiomes() {
        ravineBiome = new boolean[256];
        for (int i = 0; i < ravineBiome.length; i++) {
            Biome biome = Biome.getBiome(i);
            if (biome == null) continue;
            if (biome.getBiomeName().toLowerCase().contains("ravine")) ravineBiome[i] = true;
            if (biome.getBiomeName().toLowerCase().contains("quarry")) ravineBiome[i] = true;
        }
    }
    ...
    private void determineLandBiomes() {
        ...
        for (int i = 0; i < landBiome.length; i++) {
            if (!oceanBiome[i] && !riverBiome[i] && !beachBiome[i] && !ravineBiome[i]) {
                Biome biome = Biome.getBiome(i);
                if (biome == null) continue;
    ...
    private void setupBeachesForBiomes() {
        ...
        for (int i = 0; i < preferredBeach.length; i++) {
            ...
            // If stone beaches aren't allowed in this biome, then just use the biome... beaches are disabled...
    ...
    public void newRepair(int [] genLayerBiomes, IRealisticBiome[] jitteredBiomes, int [] biomeNeighborhood, int neighborhoodSize, float [] noise, float [] riverStrength) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            boolean canBeRiver = riverStrength[i] > 0.7 && false; //TODO: Find better way
            ...
            if (noise[i] > 61.5) {
                ...
                // check for river
                if (canBeRiver && !oceanBiome[realisticBiomeId] && !swampBiome[realisticBiomeId] && !ravineBiome[realisticBiomeId]) {
                    // make river
        ...
        for (int i = 0; i < 256; i++) {
            ...
            if (ravineBiome[biomeID]) continue;// ravines are acceptable
