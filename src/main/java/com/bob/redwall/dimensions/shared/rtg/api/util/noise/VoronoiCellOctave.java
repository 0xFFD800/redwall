...
public class VoronoiCellOctave implements CellOctave {
	...
	private double getDistance(double xDist, double yDist, double zDist) {
		switch (distanceMethod) {
		case 0:
			return Math.sqrt(xDist * xDist + yDist * yDist + zDist * zDist) / SQRT_3; // Approximation (for speed) of
																						// elucidean (regular) distance
		case 1:
	...
	public float noise(double x, double z, double frequency) {
		...
		else return ((float) valueNoise2D((int) (Math.floor(xCandidate)), (int) (Math.floor(zCandidate)), seed));
	}
	...
	public VoronoiResult eval(double x, double z) {
		...
		for (int zCur = zInt - 2; zCur <= zInt + 2; zCur++) {
			for (int xCur = xInt - 2; xCur <= xInt + 2; xCur++) {
				...
				// if closer than existing neighbot but not so close *to the candidate* as to
	...
	public double noise(double x, double y, double z, double frequency) {
		...
		else return ((double) valueNoise3D((int) (Math.floor(xCandidate)), (int) (Math.floor(yCandidate)), (int) (Math.floor(zCandidate)), seed));
