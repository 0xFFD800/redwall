...
public class DecoCollectionForestDeciduous extends DecoCollectionBase {
    ...
    private DecoHelperRandomSplit shortTrees(float noiseMin, float noiseMax) {
    	return new DecoHelperRandomSplit().setDecos(new DecoBase[] {
                shortBirchTrees(BlockUtil.getStateLog(2), BlockUtil.getStateLeaf(2), noiseMin, noiseMax),
                shortAspenTrees(tallMin, tallMax)}
            ).setChances(new int[] {15, 18});
    }
    ...
    private DecoHelperRandomSplit randomTrees() {
        return new DecoHelperRandomSplit()
            ...
            .setChances(new int[]{16, 25, 5, 5, 7, 15, 10});
    }

    private DecoHelperRandomSplit smallTrees(float noiseMin, float noiseMax) {
        return new DecoHelperRandomSplit()
            .setDecos(new DecoBase[]{
            		randomVanillaTrees(noiseMin, noiseMax), 
            		randomValourTrees(noiseMin, noiseMax, 4, BlockHandler.maple_log.getDefaultState().withProperty(BlockLog.LOG_AXIS, BlockLog.EnumAxis.Y), BlockHandler.maple_leaves.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, false), true), 
            		randomValourTrees(noiseMin, noiseMax, 4, BlockHandler.elm_log.getDefaultState().withProperty(BlockLog.LOG_AXIS, BlockLog.EnumAxis.Y), BlockHandler.elm_leaves.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, false), true),
            		randomValourTrees(noiseMin, noiseMax, 4, BlockHandler.ash_log.getDefaultState().withProperty(BlockLog.LOG_AXIS, BlockLog.EnumAxis.Y), BlockHandler.ash_leaves.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, false), true),
