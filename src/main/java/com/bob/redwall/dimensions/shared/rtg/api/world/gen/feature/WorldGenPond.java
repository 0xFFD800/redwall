...
public class WorldGenPond extends WorldGenerator {
	...
	private IBlockState surface1;
	private IBlockState surface2;

	public WorldGenPond(IBlockState fill, IBlockState surface1, IBlockState surface2) {
		this.fill = fill;
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		if (y <= 4) {
			...
			// set y to 3 below lake level so the top of the shore is the top of the lake
			// level
			y = lakeLevel - 4;
				...
				for (j2 = 0; j2 < 16; ++j2) {
					for (j1 = 0; j1 < 8; ++j1) {
						if (j1 < 4) {
							...
							} else if (willBePond[i1 * 16 + j2]) {
								if (rand.nextBoolean()) world.setBlockState(new BlockPos(x + i1, y + j1, z + j2), this.surface1, 2);
								else world.setBlockState(new BlockPos(x + i1, y + j1, z + j2), this.surface2, 2);
							}
						} else {
							// air
							if (willBePond[i1 * 16 + j2]) {
