package com.bob.redwall.dimensions.shared.rtg.api.world.gen.feature.tree.rtg;
...
public class TreeRTGQuercusRobur extends TreeRTG {
	...
	/**
	 * <b>Quercus Robur (Pedunculate Oak)</b><br>
	 * <br>
	 * <u>Relevant variables:</u><br>
	 * logBlock, logMeta, leavesBlock, leavesMeta, <s>trunkSize</s>,
	 * <s>crownSize</s>, noLeaves<br>
	 * <br>
		...
		if (i == 0) {
			...
			for (int j = 0; j <= i; ++j) {
				...
				if (!this.isReplaceable(blockpos1)) {
					...
					// String replaceBlock =
					// world.getBlockState(blockpos1).getBlock().getLocalizedName();

					// Logger.debug("Block at %d %d %d (%s) is not replaceable.", blockpos1.getX(),
					// blockpos1.getY(), blockpos1.getZ(), replaceBlock);
	...
	private boolean isReplaceable(BlockPos pos) {
		...
		return state.getBlock().isAir(state, world, pos) || state.getBlock().isLeaves(state, world, pos) || state.getBlock() == Blocks.SAPLING || state.getBlock().isWood(world, pos);
	}
