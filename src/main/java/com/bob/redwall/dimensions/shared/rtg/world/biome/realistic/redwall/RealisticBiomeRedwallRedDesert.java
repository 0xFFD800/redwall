...
public class RealisticBiomeRedwallRedDesert extends RealisticBiomeRedwallBase {
	public static Biome biome = BiomeHandler.redwall_red_desert;
	public static Biome river = BiomeHandler.redwall_river;

	public RealisticBiomeRedwallRedDesert() {
		super(biome, river);
	}
		...
		public void paintTerrain(ChunkPrimer primer, int i, int j, int x, int z, int depth, IRTGWorld rtgWorld, float[] noise, float river, Biome[] base) {
			...
			for (int k = 255; k > -1; k--) {
				...
				if (b == Blocks.AIR) {
					...
					if (cliff) {
						primer.setBlockState(x, k, z, plateauBand.getPlateauBand(rtgWorld, RealisticBiomeRedwallRedDesert.this, i, k, j));
					} else {
						if (k > 74 + grassRaise) {
							if (depth == 0) {
								if (rand.nextInt(5) == 0) {
