...
import static com.bob.redwall.dimensions.shared.rtg.api.util.MathUtils.globalToChunk;
import static com.bob.redwall.dimensions.shared.rtg.api.util.MathUtils.globalToLocal;
...
import com.bob.redwall.dimensions.shared.rtg.api.util.PlaneLocation;
...
public class OrganicBiomeGenerator {
	...
	private LimitedMap<PlaneLocation.Invariant, int[]> chunkHeights = new LimitedMap<>(64); // Keep the heights for the
																							// last 64 chunks around for
																							// a bit. We might need them
	...
	public OrganicBiomeGenerator(IRTGWorld rtgWorld) {
		...
		try {
			...
			try {
				...
				Logger.fatal("Failed to access private field 'surfaceNoise' in ChunkGeneratorOverworld. Are you in a deobfuscated environment with other mappings?", e2);
			}
		}
	}
	...
	public int[] getHeightsAt(int cx, int cz) {
		...
		for (PlaneLocation.Invariant location : chunkHeights.keySet())
			if (location.equals(inLoc))
				heights = chunkHeights.get(location);
		if (heights == null)
			heights = this.organicTerrain(cx, cz);
		return heights;
	}
