...
import com.bob.redwall.dimensions.shared.rtg.api.util.Valued;
...
//                jittered = cmr.getBiomeDataAt(pX, pZ);
    ...
    private void generateTerrain(IBiomeProviderRTG cmr, int cx, int cz, ChunkPrimer primer, IRealisticBiome biomes[], float[] noise) {
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                h = (int) noise[i * 16 + j];
				IRealisticBiome biome = biomes[i * 16 + j];

                for (int k = 0; k < 256; k++) {
                    if (k > h) {
                        if (k < 63 && !biome.noWaterBelowSeaLevel()) {
                    	...
                    	/*if(biome.isOcean()) { TODO: Return this sedementary rock style for some areas
                    		if(k < h / 2) {
                    			primer.setBlockState(i, k, j, BlockHandler.hardened_basalt.getDefaultState());
                    		} else {
                    		...
                    		/*if(k < h * (1.0F / 6.0F)) {
                    			primer.setBlockState(i, k, j, BlockHandler.hardened_basalt.getDefaultState());
                    		} else if(k < h * (2.0F / 6.0F)) {
                    			primer.setBlockState(i, k, j, BlockHandler.basalt.getDefaultState());
                    		} else if(k < h * (3.0F / 6.0F)) {
                    			primer.setBlockState(i, k, j, BlockHandler.pressurized_stone.getDefaultState());
                    		} else if(k < h * (4.0F / 6.0F)) {
                    			primer.setBlockState(i, k, j, BlockHandler.hardened_stone.getDefaultState());
                    		} else {*/
    ...
    public void populate(int x, int z) {
        if (this.alreadyDecorated.contains(new ChunkPos(x, z))) return;
	...
	private void doPopulate(int chunkX, int chunkZ) {
        ...
        TreeSet<Valued<RealisticBiomeBase>> activeBiomes = new TreeSet<Valued<RealisticBiomeBase>>();
        for (int bn = 0; bn < 256; bn++) {
            if (borderNoise[bn] > 0f) {
                ...
                activeBiomes.add(new Valued<RealisticBiomeBase>(borderNoise[bn],realisticBiome));
    ...
    private void clearDecorations(int limit) {
        //if (WorldTypeValour.chunkProvider != this) return;
    ...
    private void decorateIfOtherwiseSurrounded(IChunkProvider world, ChunkPos pos, Direction fromNewChunk) {
        ...
        if (WorldTypeRedwall.chunkProvider != this) {
	...
	private boolean chunkExists(boolean checkNeighbours, int cx, int cz) {
        ...
        if (worldObj.isChunkGeneratedAt(cx, cz)) return true;
...
//      return provideChunk(par1, par2);
    ...
    private void generateOres(IRealisticBiome rb, BlockPos pos) {
        ...
        this.rand.setSeed(worldObj.getSeed() * pos.hashCode());
