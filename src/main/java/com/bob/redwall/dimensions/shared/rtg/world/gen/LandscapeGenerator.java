...
public class LandscapeGenerator {
    ...
    private final WeakHashMap<ChunkPos,float[]> cache = new WeakHashMap<ChunkPos, float[]>();
    private PlateauBiomeCombiner plateauCombiner = new PlateauBiomeCombiner();
    private QuarryBiomeCombiner quarryCombiner = new QuarryBiomeCombiner();
    ...
    synchronized ChunkLandscape landscape(IBiomeProviderRTG cmr, int cx, int cz) {
        ...
        this.getNewerNoise(cmr, cx, cz, result);
    ...
    private synchronized void getNewerNoise(IBiomeProviderRTG cmr, int cx, int cz, ChunkLandscape landscape) {
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                ...
                // combine plateau and quarry biomes
                plateauCombiner.adjust(weightedBiomes);
                quarryCombiner.adjust(weightedBiomes);
