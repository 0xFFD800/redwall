...
public class TileEntitySmithingGeneric extends TileEntityLockable implements ITickable {
	...
	public static final int SMITHING_TIME = 200; // Time required to smith an item, in ticks; 200 is about 10 seconds
	...
	private ItemStackHandler itemStackHandler = new ItemStackHandler(SIZE) {
		...
		protected void onContentsChanged(int slot) {
			TileEntitySmithingGeneric.this.markDirty();
		}
	};

	@Override
	public String getName() {
		return this.hasCustomName() ? this.customName : "tile.smithing_generic.name";
	}
	...
	public void setInventorySlotContents(int index, ItemStack stack) {
		if (index == TileEntitySmithingGeneric.FUEL_INDEX && !stack.isEmpty() && this.fuel == 0) {
			this.fuel = 8;
	...
	public boolean isItemValidForSlot(int index, ItemStack stack) {
		return index == TileEntitySmithingGeneric.FUEL_INDEX ? SlotSmithingFuel.isValidSmithingFuel(stack) : true;
	}
	...
	public void useFuel() {
		this.fuel--;
		if (!this.itemStackHandler.getStackInSlot(TileEntitySmithingGeneric.FUEL_INDEX).isEmpty() && this.fuel == 0) {
			this.fuel = 8;
			this.decrStackSize(TileEntitySmithingGeneric.FUEL_INDEX, 1);
		}
	}
