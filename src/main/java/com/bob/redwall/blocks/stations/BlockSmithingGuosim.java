...
public class BlockSmithingGuosim extends ModBlock implements ITileEntityProvider {
	public static final PropertyDirection FACING = BlockHorizontal.FACING;

	public BlockSmithingGuosim(Material mat, String name, CreativeTabs tab, float hardness, float resistance, int harvest, String tool) {
	...
	public TileEntity createNewTileEntity(World world, int meta) {
		return new TileEntitySmithingGuosim();
	}
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing heldItem, float side, float hitX, float hitY) {
		...
		if (!world.isRemote) {
			...
			if (!(te instanceof TileEntitySmithingGuosim))
				return false;
			player.openGui(Ref.MODID, GuiHandler.GUI_SMITHING_GUOSIM_ID, world, pos.getX(), pos.getY(), pos.getZ());
			return true;
		} else {
			TileEntity te = world.getTileEntity(pos);
			if (!(te instanceof TileEntitySmithingGuosim))
			...
			Ref.NETWORK.sendToServer(new MessageUIInteractServer(Mode.OPEN_GUI_CONTAINER, GuiHandler.GUI_SMITHING_GUOSIM_ID, pos.getX(), pos.getY(), pos.getZ()));
			return true;
		}
	}
