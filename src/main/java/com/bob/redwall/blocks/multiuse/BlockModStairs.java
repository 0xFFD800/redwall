...
public class BlockModStairs extends ModBlock {

	public static final PropertyDirection FACING = BlockHorizontal.FACING;
	public static final PropertyEnum<BlockModStairs.EnumHalf> HALF = PropertyEnum.<BlockModStairs.EnumHalf>create("half", BlockModStairs.EnumHalf.class);
	public static final PropertyEnum<BlockModStairs.EnumShape> SHAPE = PropertyEnum.<BlockModStairs.EnumShape>create("shape", BlockModStairs.EnumShape.class);
	/**
	 * B: .. T: xx B: .. T: xx
	 */
	...
	public BlockModStairs(IBlockState modelState, String string, CreativeTabs tab, float hardness, float resistance, int harvest, String tool) {
		super(modelState.getBlock().getMaterial(modelState), string, tab, hardness, resistance, harvest, tool);
		this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(HALF, BlockModStairs.EnumHalf.BOTTOM).withProperty(SHAPE, BlockModStairs.EnumShape.STRAIGHT));
	...
	private static List<AxisAlignedBB> getCollisionBoxList(IBlockState bstate) {
		...
		boolean flag = bstate.getValue(HALF) == BlockModStairs.EnumHalf.TOP;
		list.add(flag ? AABB_SLAB_TOP : AABB_SLAB_BOTTOM);
		BlockModStairs.EnumShape blockstairs$enumshape = (BlockModStairs.EnumShape) bstate.getValue(SHAPE);

		if (blockstairs$enumshape == BlockModStairs.EnumShape.STRAIGHT || blockstairs$enumshape == BlockModStairs.EnumShape.INNER_LEFT || blockstairs$enumshape == BlockModStairs.EnumShape.INNER_RIGHT)
			list.add(getCollQuarterBlock(bstate));

		if (blockstairs$enumshape != BlockModStairs.EnumShape.STRAIGHT)
	...
	/**
	 * Returns a bounding box representing an eighth of a block (a block whose three
	 * dimensions are halved). Used in all stair shapes except STRAIGHT (gets added
	 * alone in the case of OUTER; alone with a quarter block in case of INSIDE).
	 */
	private static AxisAlignedBB getCollEighthBlock(IBlockState bstate) {
		EnumFacing enumfacing = (EnumFacing) bstate.getValue(FACING);
		EnumFacing enumfacing1;

		switch ((BlockModStairs.EnumShape) bstate.getValue(SHAPE)) {
		case OUTER_LEFT:
		default:
	...
	public BlockFaceShape getBlockFaceShape(IBlockAccess worldIn, IBlockState state, BlockPos pos, EnumFacing face) {
		...
		if (face.getAxis() == EnumFacing.Axis.Y) {
			return face == EnumFacing.UP == (state.getValue(HALF) == BlockModStairs.EnumHalf.TOP) ? BlockFaceShape.SOLID : BlockFaceShape.UNDEFINED;
		} else {
			BlockModStairs.EnumShape blockstairs$enumshape = (BlockModStairs.EnumShape) state.getValue(SHAPE);

			if (blockstairs$enumshape != BlockModStairs.EnumShape.OUTER_LEFT && blockstairs$enumshape != BlockModStairs.EnumShape.OUTER_RIGHT) {
	...
	/**
	 * Return an AABB (in world coords!) that should be highlighted when the player
	 * is targeting this Block
	 */
	@SideOnly(Side.CLIENT)
	...
	public boolean isTopSolid(IBlockState state) {
		return state.getValue(HALF) == BlockModStairs.EnumHalf.TOP;
	}
	...
	public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer) {
		...
		iblockstate = iblockstate.withProperty(FACING, placer.getHorizontalFacing()).withProperty(SHAPE, BlockModStairs.EnumShape.STRAIGHT);
		return facing != EnumFacing.DOWN && (facing == EnumFacing.UP || (double) hitY <= 0.5D) ? iblockstate.withProperty(HALF, BlockModStairs.EnumHalf.BOTTOM) : iblockstate.withProperty(HALF, BlockModStairs.EnumHalf.TOP);
	}
	...
	public IBlockState getStateFromMeta(int meta) {
		IBlockState iblockstate = this.getDefaultState().withProperty(HALF, (meta & 4) > 0 ? BlockModStairs.EnumHalf.TOP : BlockModStairs.EnumHalf.BOTTOM);
	...
	public int getMetaFromState(IBlockState state) {
		...
		if (state.getValue(HALF) == BlockModStairs.EnumHalf.TOP)
			i |= 4;
	...
	private static BlockModStairs.EnumShape getStairsShape(IBlockState p_185706_0_, IBlockAccess p_185706_1_, BlockPos p_185706_2_) {
		EnumFacing enumfacing = (EnumFacing) p_185706_0_.getValue(FACING);
		IBlockState iblockstate = p_185706_1_.getBlockState(p_185706_2_.offset(enumfacing));

		if (isBlockModStairs(iblockstate) && p_185706_0_.getValue(HALF) == iblockstate.getValue(HALF)) {
			...
			if (enumfacing1.getAxis() != ((EnumFacing) p_185706_0_.getValue(FACING)).getAxis() && isDifferentStairs(p_185706_0_, p_185706_1_, p_185706_2_, enumfacing1.getOpposite())) {
				if (enumfacing1 == enumfacing.rotateYCCW())
					return BlockModStairs.EnumShape.OUTER_LEFT;

				return BlockModStairs.EnumShape.OUTER_RIGHT;
			}
		}

		IBlockState iblockstate1 = p_185706_1_.getBlockState(p_185706_2_.offset(enumfacing.getOpposite()));

		if (isBlockModStairs(iblockstate1) && p_185706_0_.getValue(HALF) == iblockstate1.getValue(HALF)) {
			...
			if (enumfacing2.getAxis() != ((EnumFacing) p_185706_0_.getValue(FACING)).getAxis() && isDifferentStairs(p_185706_0_, p_185706_1_, p_185706_2_, enumfacing2)) {
				if (enumfacing2 == enumfacing.rotateYCCW())
					return BlockModStairs.EnumShape.INNER_LEFT;

				return BlockModStairs.EnumShape.INNER_RIGHT;
			}
		}

		return BlockModStairs.EnumShape.STRAIGHT;
	}
	...
	private static boolean isDifferentStairs(IBlockState p_185704_0_, IBlockAccess p_185704_1_, BlockPos p_185704_2_, EnumFacing p_185704_3_) {
		...
		return !isBlockModStairs(iblockstate) || iblockstate.getValue(FACING) != p_185704_0_.getValue(FACING) || iblockstate.getValue(HALF) != p_185704_0_.getValue(HALF);
	}

	public static boolean isBlockModStairs(IBlockState state) {
		return state.getBlock() instanceof BlockModStairs;
	}
	...
	public IBlockState withMirror(IBlockState state, Mirror mirrorIn) {
		...
		case LEFT_RIGHT:
			...
			if (enumfacing.getAxis() == EnumFacing.Axis.Z) {
				switch (blockstairs$enumshape) {
				case OUTER_LEFT:
					return state.withRotation(Rotation.CLOCKWISE_180).withProperty(SHAPE, BlockModStairs.EnumShape.OUTER_RIGHT);
				case OUTER_RIGHT:
					return state.withRotation(Rotation.CLOCKWISE_180).withProperty(SHAPE, BlockModStairs.EnumShape.OUTER_LEFT);
				case INNER_RIGHT:
					return state.withRotation(Rotation.CLOCKWISE_180).withProperty(SHAPE, BlockModStairs.EnumShape.INNER_LEFT);
				case INNER_LEFT:
					return state.withRotation(Rotation.CLOCKWISE_180).withProperty(SHAPE, BlockModStairs.EnumShape.INNER_RIGHT);
				default:
