...
public class BlockDrinkVessel extends ModBlock implements ITileEntityProvider {
		...
		if (worldIn.getTileEntity(pos) instanceof TileEntityDrinkVessel && (entityIn instanceof EntityLivingBase || entityIn instanceof EntityArrow)) {
			...
			if (te.getDrink() != null) {
				for (int i = 0; i < 30; i++) {
					...
					// ParticleSplash.Factory().createParticle(EnumParticleTypes.WATER_SPLASH.getParticleID(),
					// worldIn, pos.getX() + 0.5F, pos.getY() + 0.5F, pos.getZ() + 0.5F,
					// RANDOM.nextFloat(), RANDOM.nextFloat() * 4 + 1, RANDOM.nextFloat(), 0);
					// particle.setRBGColorF((float)((te.getDrink().getTint() >>> 16) & 0xFF) /
					// 255.0F, (float)((te.getDrink().getTint() >>> 8) & 0xFF) / 255.0F,
					// (float)(te.getDrink().getTint() & 0xFF) / 255.0F);
				}
			}
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		if (!(worldIn.getTileEntity(pos) instanceof TileEntityDrinkVessel))
			return state;
