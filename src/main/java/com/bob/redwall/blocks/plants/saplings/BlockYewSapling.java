...
public class BlockYewSapling extends BlockBush implements IGrowable {
	...
	public void generateTree(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		if (!net.minecraftforge.event.terraingen.TerrainGen.saplingGrowTree(worldIn, rand, pos))
			return;
		TreeRTG worldgenerator = new TreeRTGPinusNigra().setLogBlock(BlockHandler.yew_log.getDefaultState().withProperty(BlockLog.LOG_AXIS, EnumAxis.Y)).setLeavesBlock(BlockHandler.yew_leaves.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, false)).setMinTrunkSize(2).setMaxTrunkSize(4).setMinCrownSize(5).setMaxCrownSize(8);
		int i = 0;
		int j = 0;
		boolean flag = false;
