...
public class BlockLarchSapling extends BlockBush implements IGrowable {
	public static final PropertyInteger STAGE = PropertyInteger.create("stage", 0, 1);
	protected static final AxisAlignedBB SAPLING_AABB = new AxisAlignedBB(0.09999999403953552D, 0.0D, 0.09999999403953552D, 0.8999999761581421D, 0.800000011920929D, 0.8999999761581421D);

	public BlockLarchSapling(String name, CreativeTabs tab) {
		super();
	...
	public void generateTree(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		if (!net.minecraftforge.event.terraingen.TerrainGen.saplingGrowTree(worldIn, rand, pos))
			return;
		IBlockState log = BlockHandler.larch_log.getDefaultState().withProperty(BlockLog.LOG_AXIS, EnumAxis.Y);
		IBlockState leaves = BlockHandler.larch_leaves.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, false);
		TreeRTG worldgenerator = new TreeRTGPiceaSitchensis().setLogBlock(log).setLeavesBlock(leaves).setMinTrunkSize(4).setMaxTrunkSize(10).setMinCrownSize(6).setMaxCrownSize(14);
		int i = 0;
		int j = 0;
		boolean flag = false;
