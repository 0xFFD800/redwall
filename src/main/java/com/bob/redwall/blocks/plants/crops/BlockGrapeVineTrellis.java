...
public class BlockGrapeVineTrellis extends BlockHorizontal implements net.minecraftforge.common.IPlantable {
    ...
    public IBlockState getStateFromMeta(int meta) {
        return meta < 4  ? this.getDefaultState().withProperty(AGE, Integer.valueOf(meta)).withProperty(FACING, EnumFacing.SOUTH) : 
        	   meta < 8  ? this.getDefaultState().withProperty(AGE, Integer.valueOf(meta - 4)).withProperty(FACING, EnumFacing.WEST) :
        	   meta < 12 ? this.getDefaultState().withProperty(AGE, Integer.valueOf(meta - 8)).withProperty(FACING, EnumFacing.NORTH) :
        		           this.getDefaultState().withProperty(AGE, Integer.valueOf(meta - 12)).withProperty(FACING, EnumFacing.EAST);
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
    	if(state.getValue(AGE).intValue() != 3) return false;
