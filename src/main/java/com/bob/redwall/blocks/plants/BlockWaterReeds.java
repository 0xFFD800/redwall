	...
	public boolean canPlaceBlockAt(World worldIn, BlockPos pos) {
		...
		if (block == this) {
			...
			while (!worldIn.getBlockState(pos0).isSideSolid(worldIn, pos0, EnumFacing.DOWN) && !worldIn.getBlockState(pos0).getBlock().isAir(worldIn.getBlockState(pos0), worldIn, pos0)) {
				if (worldIn.getBlockState(pos0).getBlock() == Blocks.WATER || (worldIn.getBlockState(pos0).getBlock() == this && worldIn.getBlockState(pos0).getValue(WATERY).booleanValue()))
					return true;
				pos0 = pos0.down();
			}
			...
			while (!worldIn.getBlockState(pos1).isSideSolid(worldIn, pos1, EnumFacing.DOWN) && !worldIn.getBlockState(pos1).getBlock().isAir(worldIn.getBlockState(pos1), worldIn, pos1)) {
				if (worldIn.getBlockState(pos1).getBlock() == Blocks.WATER || (worldIn.getBlockState(pos1).getBlock() == this && worldIn.getBlockState(pos1).getValue(WATERY).booleanValue()))
					return true;
				pos1 = pos1.up();
			}
