...
public class BlockWillowSapling extends BlockBush implements IGrowable {
	...
	public BlockWillowSapling(String name, CreativeTabs tab) {
		super();
	...
	public void generateTree(World worldIn, BlockPos pos, IBlockState state, Random rand) {
		if (!net.minecraftforge.event.terraingen.TerrainGen.saplingGrowTree(worldIn, rand, pos))
			return;
		IBlockState log = BlockHandler.willow_log.getDefaultState().withProperty(BlockLog.LOG_AXIS, EnumAxis.Y);
		IBlockState leaves = BlockHandler.willow_leaves.getDefaultState().withProperty(BlockLeaves.CHECK_DECAY, false);
		TreeRTG worldgenerator = new TreeRTGSalixMyrtilloides().setLogBlock(log).setLeavesBlock(leaves).setMinTrunkSize(4).setMaxTrunkSize(10).setMinCrownSize(6).setMaxCrownSize(14);
		int i = 0;
		int j = 0;
		boolean flag = false;
