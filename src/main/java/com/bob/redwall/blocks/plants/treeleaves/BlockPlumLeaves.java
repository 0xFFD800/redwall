...
public class BlockPlumLeaves extends BlockFruitTreeLeaves implements IGrowable {
    ...
    public void getDrops(net.minecraft.util.NonNullList<ItemStack> drops, IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
        ...
        if(state.getValue(GROWTH) == EnumGrowth.FULL) drops.add(new ItemStack(Item.getByNameOrId(state.getValue(DAMSON) ? this.crop2ID : this.cropID), RANDOM.nextInt(1) + 1));
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
    	if(state.getValue(GROWTH) != EnumGrowth.FULL) return false;
