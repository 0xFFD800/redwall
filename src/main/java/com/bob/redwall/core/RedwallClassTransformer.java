...
public class RedwallClassTransformer implements IClassTransformer {
	...
	public byte[] transform(String clazz, String arg, byte[] bytes) {
		...
		/*if(clazz.equals("net.minecraft.block.BlockGravel")) {
			System.out.println(this.getClass().getName() + ".transform() about to patch class net.minecraft.block.BlockGravel (deobf)");
			return this.patchBlockGravelClassASM(clazz, bytes, false);
		}
		
		if(clazz.equals("anx")) {
			System.out.println(this.getClass().getName() + ".transform() about to patch class anx (obf)");
			return this.patchBlockGravelClassASM(clazz, bytes, true);
		}
		
		if(clazz.equals("net.minecraft.util.FoodStats")) {
			System.out.println(this.getClass().getName() + ".transform() about to patch class net.minecraft.util.FoodStats (deobf)");
			return this.patchFoodStatsClassASM(clazz, bytes, false);
		}
		
		if(clazz.equals("aci")) {
			System.out.println(this.getClass().getName() + ".transform() about to patch class aci (obf)");
			return this.patchFoodStatsClassASM(clazz, bytes, true);
		}*/
	...
	public byte[] patchItemStackClassASM(String name, byte[] bytes, boolean obfuscated) {
	    ...
	    if(obfuscated) targetMethodName = "k";
	    ...
	    while(methods.hasNext()) {
		    ...
		    //Check if this is getMaxDamage and it's method signature is ()I which means that it accepts nothing () and returns an integer (I)
		    if ((m.name.equals(targetMethodName) && m.desc.equals("()I"))) {
		    	...
		    	//Loop over the instruction set and find the instruction IRETURN which returns an integer
		    	while (iter.hasNext()) {
	...
	public byte[] patchBlockLeavesClassASM(String name, byte[] bytes, boolean obfuscated) {
		...
		String targetMethodName;
	    if(obfuscated) targetMethodName = "c";
	    ...
	    if(srg) targetMethodName = "func_176221_a";
	...
	public byte[] patchBlockLeafClassASM(String name, byte[] bytes, boolean obfuscated) {
		String targetMethodName = "";
	    if(obfuscated) targetMethodName = "b";
	...
	public byte[] patchBlockColors$4ClassASM(String name, byte[] bytes, boolean obfuscated) {
	    ...
	    if(obfuscated) targetMethodName = "a";
	    else targetMethodName = "colorMultiplier";
	    if(obfuscated) targetMethodDesc = "(Latj;Laju;Lco;I)I";
	...
	public byte[] patchMinecraftClassASM(String name, byte[] bytes, boolean obfuscated) {
		String targetMethodName;
		String targetMethodDesc;
	    if(obfuscated) targetMethodName = "aE";
	    else targetMethodName = "processKeyBinds";
	    if(obfuscated) targetMethodDesc = "()V";
	...
	/*public byte[] patchBlockGravelClassASM(String name, byte[] bytes, boolean obfuscated) {
		String targetMethodName = "";
	    if(obfuscated) targetMethodName = "a";
	    else targetMethodName = "getItemDropped";
	    boolean finished = false;
	    ...
	    while(methods.hasNext()) {
		    ...
		    if ((m.name.equals(targetMethodName) && m.desc.equals(obfuscated ? "(Latj;Ljava/util/Random;I)Lafg;" : "(Lnet/minecraft/block/state/IBlockState;Ljava/util/Random;I)Lnet/minecraft/item/Item;"))) {
		    	System.out.println(this.getClass().getName() + ".patchBlockGravelClassASM() inside target method " + targetMethodName); 
		    	AbstractInsnNode currentNode = null;
		    	AbstractInsnNode targetNode = null;
		    	...
		    	while (iter.hasNext()) {
			    	...
			    	if (currentNode instanceof FrameNode) {
			    		targetNode = currentNode;
			    		found++;
			    		if(found > 0) break;
			    	}
		    	...
		    	System.out.println(this.getClass().getName() + ".patchBlockGravelClassASM() patched method " + targetMethodName);
		    	finished = true;
		    	break;
		    }
	    }
	    ...
	    if(!finished) {
	    	System.out.println("Couldn't find method " + targetMethodName + ", trying again with func_180660_a.");
		    targetMethodName = "func_180660_a";
	...
	public byte[] patchFoodStatsClassASM(String name, byte[] bytes, boolean obfuscated) {
		String targetMethodName = "";
		String targetMethodDesc = "";
	    if(obfuscated) {
	    	targetMethodName = "a";
	    	targetMethodDesc = "(Laax;)V";
	    } else {
	    ...
	    while(methods.hasNext()) {
		    ...
		    if ((m.name.equals(targetMethodName) && m.desc.equals(targetMethodDesc))) {
		    	System.out.println(this.getClass().getName() + ".patchFoodStatsClassASM() inside target method " + targetMethodName); 
		    	AbstractInsnNode currentNode = null;
		    	AbstractInsnNode targetNode = null;
		    	...
		    	while (iter.hasNext()) {
			    	...
			    	if(found) m.instructions.remove(currentNode);
			    	if (currentNode instanceof LineNumberNode && ((LineNumberNode)currentNode).line == 59) {
			    		targetNode = currentNode;
			    		found = true;
			    	} else if (currentNode instanceof LineNumberNode && ((LineNumberNode)currentNode).line == 61) {
			    		break;
			    	}
		    	}
		    	...
		    	toInject.add(new VarInsnNode(ISTORE, 3));
		    	toInject.add(new VarInsnNode(ALOAD, 1));
		    	toInject.add(new MethodInsnNode(INVOKESTATIC, "com/bob/valour/ValourUtils", "updateFoodStats", targetMethodDesc, false));
		    	
		    	m.instructions.insertBefore(targetNode, toInject);
		    	
		    	System.out.println(this.getClass().getName() + ".patchFoodStatsClassASM() patched method " + targetMethodName);
		    	finished = true;
		    	break;
		    }
	    }
	    ...
	    if(!finished) {
	    	System.out.println("Couldn't find method " + targetMethodName + ", trying again with func_75118_a.");
		    targetMethodName = "func_75118_a";
