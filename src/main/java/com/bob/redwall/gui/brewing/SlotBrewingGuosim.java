...
public class SlotBrewingGuosim extends Slot {
	private final InventoryCrafting craftMatrix;
	private final EntityPlayer player;
	private int amountCrafted;
	private TileEntityBrewingGuosim te;

	public SlotBrewingGuosim(EntityPlayer player, InventoryCrafting craftingInventory, IInventory inventoryIn, int slotIndex, int xPosition, int yPosition, TileEntityBrewingGuosim te2) {
	...
	protected void onCrafting(ItemStack stack) {
		if (this.amountCrafted > 0) {
			...
			cap.set(Faction.FacList.GUOSIM, FactionCap.FacStatType.BREW, cap.get(Faction.FacList.GUOSIM, FactionCap.FacStatType.BREW) + 5.0F, true);
			RedwallUtils.applyFoodModifiers(this.player, stack, cap.get(Faction.FacList.GUOSIM, FactionCap.FacStatType.BREW));
		}
	...
	public boolean canTakeStack(EntityPlayer player) {
		if (this.te.getBrewingFinished()) return true;
