...
public class SlotBrewingRedwall extends Slot {
	private final InventoryCrafting craftMatrix;
	private final EntityPlayer player;
	private int amountCrafted;
	private TileEntityBrewingRedwall te;

	public SlotBrewingRedwall(EntityPlayer player, InventoryCrafting craftingInventory, IInventory inventoryIn, int slotIndex, int xPosition, int yPosition, TileEntityBrewingRedwall te2) {
	...
	public boolean canTakeStack(EntityPlayer player) {
		...
		if (this.te.getBrewingTime() == -1) {
			this.te.setBrewingTime(TileEntityBrewingRedwall.BREWING_TIME);
			this.te.brewStack = this.getStack();
			this.te.markDirty();
			net.minecraftforge.common.ForgeHooks.setCraftingPlayer(player);
			NonNullList<ItemStack> nonnulllist = CraftingHandler.BrewingRedwall.getInstance().getRemainingItems(this.craftMatrix, player.world);
