...
public class SlotSmithingRedwall extends Slot {
	private final InventoryCrafting craftMatrix;
	private final EntityPlayer player;
	private int amountCrafted;
	private TileEntitySmithingRedwall te;

	public SlotSmithingRedwall(EntityPlayer player, InventoryCrafting craftingInventory, IInventory inventoryIn, int slotIndex, int xPosition, int yPosition, TileEntitySmithingRedwall te) {
	...
	protected void onCrafting(ItemStack stack) {
		if (this.amountCrafted > 0) {
			...
			cap.set(Faction.FacList.REDWALL, FactionCap.FacStatType.SMITH, cap.get(Faction.FacList.REDWALL, FactionCap.FacStatType.SMITH) + 5.0F, true);
			RedwallUtils.applyEquipmentModifiers(this.player, stack, cap.get(Faction.FacList.REDWALL, FactionCap.FacStatType.SMITH));
		}
